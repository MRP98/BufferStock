...
class model_bufferstock():
    ...
    def setup(self):
        ...
        # Income parameters
        par.G = 1.03 # Age-invariant deterministic drift in income
        par.L = np.ones((par.T)) # Age-specific deterministic drift in income (If ones, then no life cycle)
        ...
        par.R = 1.04 # Return on savings
        par.lambdaa = 0.0  # Maximum borrowing limit
    ...
    def life_cycle_setup(self):
        ...
        par.simT = par.T # Number of periods in simulation = terminal age
        par.simlifecycle = 1 # Finite horizen model
